...
function importData() {
    ...
    const parseValToInt = (data) => { //in dataset type of "value" is string, so this code parses it to int
        let array = _.cloneDeep(data.children);
        array.forEach(elem => {
    ...
    dataset.forEach((item, idx) => d3.json(BASE_URL + URLS[idx])
        .then(response => item.data = parseValToInt(response)));
    return dataset;
}
